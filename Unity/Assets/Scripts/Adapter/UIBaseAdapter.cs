...
public class UIBaseAdapter : CrossBindingAdaptor
{
    ...
    //为了完整实现Widget的所有特性，这个Adapter还得扩展，这里只抛砖引玉，只实现了最常用的Awake, Start和Update
    public class Adaptor : UIBase, CrossBindingAdaptorType
    {
        ...
        IMethod mPopUpMethod;
        bool mPopUpMethodGot;
        public override void PopUp()
        {
            if (!mPopUpMethodGot)
            {
                mPopUpMethod = instance.Type.GetMethod("PopUp", 0);
                mPopUpMethodGot = true;
            }

            if (mPopUpMethod != null)
            {
                appdomain.Invoke(mPopUpMethod, instance, null);
            }
        }
